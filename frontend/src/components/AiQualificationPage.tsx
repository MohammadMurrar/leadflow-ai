import { AlertTriangle, Bot, CheckCircle2, Clock3, Gauge, Search, X } from 'lucide-react'
...
export default function AiQualificationPage({
    ...
    const cards = [
        ...
        { label: 'Successfully qualified', value: summary?.qualifiedLeads ?? 0, icon: CheckCircle2 },
        { label: 'Not qualified', value: failed, icon: AlertTriangle },
        { label: 'Average AI score', value: summary ? summary.averageAiScore.toFixed(1) : '0.0', icon: Gauge },
    ]

    return (
        <main className="px-4 py-7 sm:px-6 lg:px-8">
            <section className="mb-7">
                <p className="text-sm font-semibold text-primary">AI operations</p>
                ...
                <div className="border-b border-slate-100 p-5 sm:p-6">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                            <label className="relative sm:w-64"><span className="sr-only">Search qualification queue</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type="search" maxLength={100} value={searchInput} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search qualification records..." className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-9 text-sm outline-none focus:border-primary-300 focus:ring-4 focus:ring-primary-100" />{searchInput && <button type="button" aria-label="Clear qualification search" onClick={() => onSearchChange('')} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 hover:bg-slate-200"><X className="h-3.5 w-3.5" /></button>}</label>
                            <label><span className="sr-only">Filter by qualification state</span><select value={qualificationState} onChange={(event) => onStateChange(event.target.value as QualificationState | '')} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:border-primary-300 focus:ring-4 focus:ring-primary-100"><option value="">All qualification states</option><option value="PROCESSING">Processing</option><option value="SUCCESSFULLY_QUALIFIED">Successfully qualified</option><option value="FAILED">Failed</option></select></label>
                            <label><span className="sr-only">Sort qualification queue</span><select value={sort} onChange={(event) => onSortChange(event.target.value as LeadSort)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:border-primary-300 focus:ring-4 focus:ring-primary-100"><option value="createdAt,desc">Newest first</option><option value="createdAt,asc">Oldest first</option><option value="qualificationScore,desc">Highest AI score</option></select></label>
                            {hasFilters && <button type="button" onClick={() => { onSearchChange(''); onStateChange('') }} className="h-10 text-sm font-semibold text-primary hover:text-primary-700">Clear filters</button>}
                        </div>
                    </div>
                    <span className="mt-3 block h-4 text-xs font-medium text-primary" role="status" aria-live="polite">{isFetching && !isLoading && isPlaceholderData ? 'Updating qualification results...' : ''}</span>
                </div>

                {isLoading && <div className="px-6 py-16 text-center"><Clock3 className="mx-auto mb-3 h-6 w-6 animate-spin text-primary" /><p className="text-sm text-slate-500">Loading qualification records...</p></div>}

        ...
        <main className="px-4 py-7 sm:px-6 lg:px-8">
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                            ...
                            <label>
                                ...
                                <select
                                    ...
                                    className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:border-primary-300 focus:ring-4 focus:ring-primary-100"
                                >
                            ...
                            {hasFilters && (
                                <button
                                    ...
                                    className="h-10 text-sm font-semibold text-primary hover:text-primary-700"
                                >
                                    Clear filters
                                </button>
                            )}
                ...
                {!isLoading && leads.length > 0 && (
                    <>
                    <div className="divide-y divide-slate-100 md:hidden" aria-label="Lead records">
                        {leads.map((lead) => <LeadMobileRecord key={lead.id} lead={lead} currency={currency} context="leads" />)}
                    </div>
                                    ...
                                    <tr key={lead.id} className="transition hover:bg-slate-50/70">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary-50 text-xs font-bold text-primary-700">{getInitials(lead.fullName)}</div>
                                            ...
                                            {lead.qualificationScore !== null ? (
                                                <div className="flex items-center gap-3">
                                                    <div className="h-1.5 w-14 overflow-hidden rounded-full bg-slate-100">
                                                        <div style={{ width: `${Math.min(Math.max(lead.qualificationScore, 0), 100)}%` }} className="h-full rounded-full bg-primary" />
                                                    </div>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${lead.status === 'QUALIFIED' ? 'bg-emerald-50 text-emerald-700' : lead.status === 'AUTOMATION_FAILED' ? 'bg-rose-50 text-rose-700' : lead.status === 'WON' ? 'bg-primary-50 text-primary-700' : lead.status === 'LOST' ? 'bg-slate-100 text-slate-600' : 'bg-blue-50 text-blue-700'}`}>

...
export default function ResetPasswordPage() {
  ...
  return <AuthLayout><section className="auth-card" aria-labelledby="reset-heading"><p className="auth-eyebrow">Secure reset</p><h1 id="reset-heading" className="auth-heading">Create a new password</h1><p className="auth-copy">Choose a strong password for your administrator account.</p><form className="mt-7 space-y-5" onSubmit={submit}><PasswordField id="new-password" label="New password" placeholder="Create a new password" hint="Use 12–128 characters with uppercase and lowercase letters, a decimal digit, and a non-alphanumeric character. Do not use leading or trailing whitespace, control characters, your account email or its local part, or your current password." value={password} setValue={setPassword} visible={showPassword} setVisible={setShowPassword} inputRef={passwordRef} describedBy={error ? 'password-requirements reset-error' : 'password-requirements'} invalid={Boolean(error)} /><ul id="password-requirements" className="grid gap-2 text-xs text-slate-600 sm:grid-cols-2" aria-label="Password requirements">{rules.map(([key, label]) => <li key={key} className="flex items-center gap-2">{policy[key] ? <Check className="h-4 w-4 text-emerald-600" aria-hidden="true" /> : <Circle className="h-4 w-4 text-slate-400" aria-hidden="true" />}<span>{label}: {policy[key] ? 'met' : 'not met'}</span></li>)}</ul><PasswordField id="confirm-password" label="Confirm new password" placeholder="Re-enter your new password" hint="Enter the same password again." value={confirmation} setValue={setConfirmation} visible={showConfirmation} setVisible={setShowConfirmation} invalid={Boolean(error && password !== confirmation)} describedBy={error ? 'reset-error' : undefined} inputRef={confirmationRef} />{error && <p ref={errorRef} tabIndex={-1} id="reset-error" className="auth-alert" role="alert">{error}</p>}<button type="submit" disabled={pending} className="auth-submit">{pending ? 'Updating password…' : 'Update password'}</button></form></section></AuthLayout>
}

interface PasswordFieldProps { id: string; label: string; placeholder: string; hint: string; value: string; setValue: (value: string) => void; visible: boolean; setVisible: (value: boolean) => void; invalid: boolean; describedBy?: string; inputRef?: Ref<HTMLInputElement> }
function PasswordField({ id, label, placeholder, hint, value, setValue, visible, setVisible, invalid, describedBy, inputRef }: PasswordFieldProps) {
  const hintId = `${id}-hint`
  const descriptionIds = describedBy ? `${hintId} ${describedBy}` : hintId
  return <div><label htmlFor={id} className="auth-label">{label}</label><span className="auth-input-wrap"><KeyRound className="auth-input-icon" /><input ref={inputRef} id={id} type={visible ? 'text' : 'password'} autoComplete="new-password" required maxLength={128} value={value} onChange={(event) => setValue(event.target.value)} placeholder={placeholder} aria-invalid={invalid} aria-describedby={descriptionIds} className="auth-input pr-12" /><button type="button" className="auth-visibility" onClick={() => setVisible(!visible)} aria-label={visible ? `Hide ${label.toLowerCase()}` : `Show ${label.toLowerCase()}`}>{visible ? <EyeOff /> : <Eye />}</button></span><span id={hintId} className="auth-hint">{hint}</span></div>
}

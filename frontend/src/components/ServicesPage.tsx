...
export default function ServicesPage() {
    ...
    return <main className="min-w-0 px-4 py-7 sm:px-6 lg:px-8">
        <section className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div><p className="text-sm font-semibold text-indigo-600">Catalog management</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Services</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">Manage the services available for new leads while preserving every historical lead snapshot.</p></div>
            <button type="button" onClick={(event) => openCreate(event.currentTarget)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white shadow-sm shadow-indigo-200 hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200"><Plus className="h-4 w-4" />Create Service</button>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm" aria-busy={query.isFetching}>
            <div className="border-b border-slate-100 p-5 sm:p-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                <label className="relative flex-1"><span className="sr-only">Search services</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input type="search" maxLength={100} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search name or description..." className="h-10 w-full rounded-xl border border-slate-200 pl-9 pr-9 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" />{searchInput && <button type="button" aria-label="Clear service search" onClick={() => setSearchInput('')} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button>}</label>
                <label><span className="sr-only">Filter services by status</span><select value={status} onChange={(event) => { setStatus(event.target.value as StatusFilter); setPage(0) }} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:ring-4 focus:ring-indigo-100"><option value="">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label>
                <label><span className="sr-only">Sort services</span><select value={sort} onChange={(event) => { setSort(event.target.value as ServiceSort); setPage(0) }} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:ring-4 focus:ring-indigo-100"><option value="name,asc">Name A–Z</option><option value="name,desc">Name Z–A</option><option value="createdAt,desc">Newest</option><option value="createdAt,asc">Oldest</option></select></label>
                {hasControls && <button type="button" onClick={() => { setSearchInput(''); setStatus(''); setSort('name,asc'); setPage(0) }} className="h-10 text-sm font-semibold text-indigo-600">Clear filters</button>}
            </div><span className="mt-3 block h-4 text-xs font-medium text-indigo-600" role="status" aria-live="polite">{query.isFetching && !query.isLoading ? 'Updating services...' : ''}</span></div>
            ...
            {query.isLoading && !displayedPage && <div className="px-6 py-16 text-center"><Clock3 className="mx-auto h-7 w-7 animate-spin text-indigo-500" /><p className="mt-3 text-sm text-slate-500">Loading services...</p></div>}
            {query.isError && !displayedPage && <div className="px-6 py-16 text-center"><AlertTriangle className="mx-auto h-7 w-7 text-rose-500" /><p className="mt-3 text-sm text-slate-600">Services could not be loaded.</p><button type="button" onClick={() => query.refetch()} className="mt-4 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Retry</button></div>}
            ...
            {displayedPage && displayedPage.content.length > 0 && <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left"><thead><tr className="border-b border-slate-100 bg-slate-50/60 text-xs font-semibold uppercase tracking-wider text-slate-400"><th className="px-6 py-4">Service</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Created</th><th className="px-6 py-4">Updated</th><th className="px-6 py-4"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-slate-100">{displayedPage.content.map((offering) => <tr key={offering.id} className="hover:bg-slate-50/60"><td className="px-6 py-4"><p className="font-semibold text-slate-800">{offering.name}</p><p className="mt-1 max-w-xl whitespace-pre-wrap text-sm text-slate-500">{offering.description ?? 'No description'}</p></td><td className="px-6 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${offering.active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{offering.active ? 'Active' : 'Inactive'}</span></td><td className="px-6 py-4 text-sm text-slate-500">{formatDate(offering.createdAt)}</td><td className="px-6 py-4 text-sm text-slate-500">{formatDate(offering.updatedAt)}</td><td className="px-6 py-4"><div className="flex justify-end gap-2"><button type="button" onClick={(event) => openEdit(offering, event.currentTarget)} className="rounded-lg border border-slate-200 p-2 text-slate-600 hover:bg-slate-50" aria-label={`Edit ${offering.name}`}><Edit3 className="h-4 w-4" /></button>{offering.active ? <button type="button" onClick={(event) => { triggerRef.current = event.currentTarget; setActionError(null); setConfirming(offering) }} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50">Deactivate</button> : <button type="button" disabled={stateMutation.isPending} onClick={() => stateMutation.mutate({ offering, active: true })} className="rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">Reactivate</button>}</div></td></tr>)}</tbody></table></div>}

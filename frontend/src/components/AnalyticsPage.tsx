                ...
                <>
                    <article className="mb-6 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <div className="h-80 w-full" role="img" aria-label={`Trend with ${data.totalLeads} total leads and ${data.qualifiedLeads} successfully qualified leads in ${rangeLabels[range].toLowerCase()}`}>
                            <ResponsiveContainer width="100%" height="100%"><AreaChart data={data.performance} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}><defs><linearGradient id="analyticsTotalGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#142A43" stopOpacity={0.22} /><stop offset="95%" stopColor="#142A43" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="periodStart" tickFormatter={(value: string) => formatPeriod(value, monthly)} interval={performanceTickInterval} minTickGap={14} axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} /><Tooltip labelFormatter={(label) => formatPeriod(String(label), monthly, true)} formatter={(value, name) => [Number(value).toLocaleString(), name === 'totalLeads' ? 'Total leads' : 'Successfully qualified']} contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 8px 24px rgba(15,23,42,0.08)' }} /><Area type="linear" dataKey="totalLeads" name="Total leads" stroke="#142A43" strokeWidth={2} fill="url(#analyticsTotalGradient)" /><Area type="linear" dataKey="qualifiedLeads" name="Successfully qualified" stroke="#10b981" strokeWidth={2} fill="transparent" /></AreaChart></ResponsiveContainer>
                        </div>

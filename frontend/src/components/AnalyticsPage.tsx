                    ...
                    {items.map((item) => (
                        <li key={item.name}>
                            ...
                            <div className="h-2 overflow-hidden rounded-full bg-slate-100" aria-hidden="true">
                                <div className="h-full rounded-full bg-indigo-500" style={{ width: `${(item.count / max) * 100}%` }} />
                            </div>
                        </li>
                    ))}
                </ul>
...
export default function AnalyticsPage({
    ...
    if (isLoading && !data) {
        return <main className="px-4 py-7 sm:px-6 lg:px-8"><div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-slate-200 bg-white"><div className="text-center"><BarChart3 className="mx-auto h-8 w-8 animate-pulse text-indigo-500" /><p className="mt-3 text-sm text-slate-500">Loading analytics...</p></div></div></main>
    }

    if (isError && !data) {
        return <main className="px-4 py-7 sm:px-6 lg:px-8"><div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-rose-100 bg-white"><div className="text-center"><AlertTriangle className="mx-auto h-8 w-8 text-rose-500" /><h1 className="mt-3 text-lg font-bold text-slate-900">Analytics could not be loaded</h1><p className="mt-1 text-sm text-slate-500">The request did not complete. Your selected range is preserved.</p><button type="button" onClick={onRetry} className="mt-5 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white outline-none focus:ring-4 focus:ring-indigo-200">Retry</button></div></div></main>
    }
    ...
    return (
        <main className="min-w-0 px-4 py-7 sm:px-6 lg:px-8">
            <section className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <div>
                    <p className="text-sm font-semibold text-indigo-600">Decision intelligence</p>
                ...
                <div className="flex items-center gap-3 self-start sm:self-auto">
                    <span className="min-w-20 text-right text-xs font-semibold text-indigo-600" role="status" aria-live="polite">{isFetching && !isLoading ? `Updating ${rangeLabels[range].toLowerCase()}...` : ''}</span>
                    <label><span className="sr-only">Analytics date range</span><select value={range} onChange={(event) => onRangeChange(event.target.value as DashboardRange)} className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-600 shadow-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100"><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option><option value="all">All time</option></select></label>
                </div>
            </section>
            ...
            <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label={`${rangeLabels[range]} analytics summary`} aria-busy={isPlaceholderData}>
                {cards.map((card) => <article key={card.label} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${card.style}`}><card.icon className="h-5 w-5" /></div><p className="mt-4 text-sm font-medium text-slate-500">{card.label}</p><p className="mt-1 text-2xl font-bold text-slate-950">{card.value}</p><p className="mt-1 text-xs leading-5 text-slate-400">{card.detail}</p></article>)}
            </section>
                ...
                <>
                    <article className="mb-6 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <div className="h-80 w-full" role="img" aria-label={`Trend with ${data.totalLeads} total leads and ${data.qualifiedLeads} successfully qualified leads in ${rangeLabels[range].toLowerCase()}`}>
                            <ResponsiveContainer width="100%" height="100%"><AreaChart data={data.performance} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}><defs><linearGradient id="analyticsTotalGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.22} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="periodStart" tickFormatter={(value: string) => formatPeriod(value, monthly)} interval={performanceTickInterval} minTickGap={14} axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} /><Tooltip labelFormatter={(label) => formatPeriod(String(label), monthly, true)} formatter={(value, name) => [Number(value).toLocaleString(), name === 'totalLeads' ? 'Total leads' : 'Successfully qualified']} contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 8px 24px rgba(15,23,42,0.08)' }} /><Area type="linear" dataKey="totalLeads" name="Total leads" stroke="#6366f1" strokeWidth={2} fill="url(#analyticsTotalGradient)" /><Area type="linear" dataKey="qualifiedLeads" name="Successfully qualified" stroke="#10b981" strokeWidth={2} fill="transparent" /></AreaChart></ResponsiveContainer>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-5 text-xs font-medium text-slate-500"><span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-indigo-500" />Total leads</span><span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" />Successfully qualified</span></div>
                    </article>

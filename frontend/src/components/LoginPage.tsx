...
export default function LoginPage({ onSubmit, isPending, error }: LoginPageProps) {
  ...
  return <AuthLayout><section className="auth-card" aria-labelledby="login-heading"><p className="auth-eyebrow">Administrator access</p><h1 id="login-heading" className="auth-heading">Welcome back</h1><p className="auth-copy">Sign in to manage inquiries and keep your pipeline moving.</p><form className="mt-8 space-y-5" onSubmit={handleSubmit}><label className="auth-label">Email address<span className="auth-input-wrap"><Mail className="auth-input-icon" /><input ref={emailRef} name="email" type="email" autoComplete="username" required maxLength={254} value={email} onChange={(event) => setEmail(event.target.value)} disabled={isPending} className="auth-input" /></span></label><div><div className="flex items-center justify-between"><label htmlFor="login-password" className="auth-label !mb-0">Password</label><Link to="/forgot-password" className="auth-link text-sm">Forgot password?</Link></div><span className="auth-input-wrap mt-2"><LockKeyhole className="auth-input-icon" /><input id="login-password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required maxLength={256} value={password} onChange={(event) => setPassword(event.target.value)} disabled={isPending} className="auth-input pr-12" /><button type="button" className="auth-visibility" onClick={() => setShowPassword((current) => !current)} aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff /> : <Eye />}</button></span></div>{error && <p ref={alertRef} tabIndex={-1} className="auth-alert" role="alert">{error}</p>}<button type="submit" disabled={isPending} className="auth-submit">{isPending ? 'Signing in…' : 'Sign in securely'}</button></form></section></AuthLayout>
}
